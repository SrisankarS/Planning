...
public interface MovieShowsRepository extends JpaRepository<MovieShowsEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start > CURRENT_DATE group by id", nativeQuery = true)
    List<MovieShowsEntity> findAllUpComing();

    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start <= CURRENT_DATE AND ms.show_end >= CURRENT_DATE group by id", nativeQuery = true)
    List<MovieShowsEntity> findAllNowPlaying();

    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_end  >= CURRENT_DATE group by id", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start > CURRENT_DATE LIMIT :records group by id", nativeQuery = true)
    List<MovieShowsEntity> findFewUpComing(@Param("records") final int records);

    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start <= CURRENT_DATE AND ms.show_end >= CURRENT_DATE LIMIT :records group by id", nativeQuery = true)
